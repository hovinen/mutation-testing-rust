...
#[derive(Clone)]
pub(crate) struct IfConditionToTrueMutator;

impl Mutator for IfConditionToTrueMutator {
    fn perform(&self, body: &mut FuncBody, index: usize) {
        if let Instruction::BrIf(depth) = body.code().elements()[index] {
            let elements = body.code_mut().elements_mut();
            elements[index] = Instruction::Br(depth);
            elements.insert(index, Instruction::Drop);
        }
    }
    ...
    fn describe(&self) -> String {
        String::from("IfConditionToTrue")
    }
}

impl IfConditionToTrueMutator {
...
mod tests {
    ...
    fn perform_writes_new_instruction_at_index_0() {
        let subject = IfConditionToTrueMutator;
    ...
    #[test]
    fn perform_writes_new_instruction_with_nonzero_depth_at_index_0() {
        let subject = IfConditionToTrueMutator;
        let mut body = FuncBodyBuilder::with_callback(Identity)
            .with_instructions(Instructions::new(vec![Instruction::BrIf(5)]))
            .build();
    ...
    fn find_identifies_mutation_at_index_1() {
        ...
        assert_eq!(
            module.code_section().unwrap().bodies()[0].code().elements(),
            vec![Instruction::I32Add, Instruction::Drop, Instruction::Br(0)]
        );
    }
